...
SS_MSG(source_constraints,
    EN("Training sources"), JA("学習ソース"), ZH_HANS("训练来源权重"),
    ZH_HANT("訓練來源權重"), KO("학습 소스"), DE("Trainingsquellen"),
    FR("Sources d'entraînement"), ES("Fuentes de entrenamiento"), PT("Fontes de treino"),
    IT("Fonti di addestramento"), NL("Trainingsbronnen"), RU("Источники обучения"),
    TR("Eğitim kaynakları"));
SS_MSG(source_reset,
    EN("Reset stages"), JA("段階をリセット"), ZH_HANS("重置阶段"), ZH_HANT("重設階段"),
    KO("단계 초기화"), DE("Phasen zurücksetzen"), FR("Réinitialiser les phases"),
    ES("Restablecer etapas"), PT("Redefinir fases"), IT("Ripristina fasi"),
    NL("Fasen herstellen"), RU("Сбросить этапы"), TR("Aşamaları sıfırla"));
SS_MSG(source_stage,
    EN("Stage {0}: {1}% - {2}%"), JA("段階 {0}: {1}% - {2}%"),
    ZH_HANS("阶段 {0}：{1}% - {2}%"), ZH_HANT("階段 {0}：{1}% - {2}%"),
    KO("단계 {0}: {1}% - {2}%"), DE("Phase {0}: {1}% - {2}%"),
    FR("Phase {0} : {1}% - {2}%"), ES("Etapa {0}: {1}% - {2}%"),
    PT("Fase {0}: {1}% - {2}%"), IT("Fase {0}: {1}% - {2}%"),
    NL("Fase {0}: {1}% - {2}%"), RU("Этап {0}: {1}% - {2}%"),
    TR("Aşama {0}: {1}% - {2}%"));
SS_MSG(source_until,
    EN("End (%)"), JA("終了 (%)"), ZH_HANS("结束于 (%)"), ZH_HANT("結束於 (%)"),
    KO("종료 (%)"), DE("Ende (%)"), FR("Fin (%)"), ES("Fin (%)"), PT("Fim (%)"),
    IT("Fine (%)"), NL("Einde (%)"), RU("Конец (%)"), TR("Bitiş (%)"));
SS_MSG(source_remove_stage,
    EN("Remove stage"), JA("段階を削除"), ZH_HANS("删除阶段"), ZH_HANT("刪除階段"),
    KO("단계 삭제"), DE("Phase entfernen"), FR("Supprimer la phase"),
    ES("Eliminar etapa"), PT("Remover fase"), IT("Rimuovi fase"),
    NL("Fase verwijderen"), RU("Удалить этап"), TR("Aşamayı kaldır"));
SS_MSG(source_camera,
    EN("Camera group"),
    JA("カメラグループ"),
    ZH_HANS("相机内参组"),
    ZH_HANT("相機內參組"),
    KO("카메라 그룹"),
    DE("Kameragruppe"),
    FR("Groupe de caméras"),
    ES("Grupo de cámaras"),
    PT("Grupo de câmeras"),
    IT("Gruppo di fotocamere"),
    NL("Cameragroep"),
    RU("Группа камер"),
    TR("Kamera grubu"));
SS_MSG(source_camera_label,
    EN("Camera {0} | {1} | {2} x {3}"),
    JA("カメラ {0}｜{1}｜{2} × {3}"),
    ZH_HANS("相机 {0}｜{1}｜{2} × {3}"),
    ZH_HANT("相機 {0}｜{1}｜{2} × {3}"),
    ...
    ES("Cámara {0} | {1} | {2} x {3}"),
    PT("Câmera {0} | {1} | {2} x {3}"),
    IT("Fotocamera {0} | {1} | {2} x {3}"),
    NL("Camera {0} | {1} | {2} x {3}"),
    RU("Камера {0} | {1} | {2} x {3}"),
    TR("Kamera {0} | {1} | {2} x {3}"));
SS_MSG(snapshot_coordinates,
    EN("Coordinate system"), JA("座標系"), ZH_HANS("坐标系"), ZH_HANT("座標系"),
    KO("좌표계"), DE("Koordinatensystem"), FR("Système de coordonnées"),
    ES("Sistema de coordenadas"), PT("Sistema de coordenadas"), IT("Sistema di coordinate"),
    NL("Coördinatensysteem"), RU("Система координат"), TR("Koordinat sistemi"));
SS_MSG(snapshot_z_up,
    EN("Z-up (Spirula)"), JA("Z 上（Spirula）"), ZH_HANS("Z 向上（Spirula）"), ZH_HANT("Z 向上（Spirula）"),
    KO("Z 위 (Spirula)"), DE("Z oben (Spirula)"), FR("Z vers le haut (Spirula)"),
    ES("Z arriba (Spirula)"), PT("Z para cima (Spirula)"), IT("Z in alto (Spirula)"),
    NL("Z omhoog (Spirula)"), RU("Z вверх (Spirula)"), TR("Z yukarı (Spirula)"));
SS_MSG(snapshot_y_up,
    EN("Y-up"), JA("Y 上"), ZH_HANS("Y 向上"), ZH_HANT("Y 向上"), KO("Y 위"),
    DE("Y oben"), FR("Y vers le haut"), ES("Y arriba"), PT("Y para cima"), IT("Y in alto"),
    NL("Y omhoog"), RU("Y вверх"), TR("Y yukarı"));
SS_MSG(snapshot_supersplat,
    EN("SuperSplat (Y-up)"), JA("SuperSplat（Y 上）"), ZH_HANS("SuperSplat（Y 向上）"), ZH_HANT("SuperSplat（Y 向上）"),
    KO("SuperSplat (Y 위)"), DE("SuperSplat (Y oben)"), FR("SuperSplat (Y vers le haut)"),
    ES("SuperSplat (Y arriba)"), PT("SuperSplat (Y para cima)"), IT("SuperSplat (Y in alto)"),
    NL("SuperSplat (Y omhoog)"), RU("SuperSplat (Y вверх)"), TR("SuperSplat (Y yukarı)"));
SS_MSG(snapshot_coordinates_help,
    EN("Changes the preview up axis and the coordinate conversion used by Generate snapshot. SuperSplat also accounts for its PLY import rotation."),
    JA("プレビューの上軸とスナップショットの座標変換を変更します。SuperSplat では PLY 読み込み時の回転も処理します。"),
    ZH_HANS("同时改变预览的向上轴和生成快照使用的坐标转换。SuperSplat 还会处理其 PLY 导入旋转。"),
    ZH_HANT("同時改變預覽的向上軸與產生快照使用的座標轉換。SuperSplat 也會處理其 PLY 匯入旋轉。"),
    KO("미리보기의 위쪽 축과 스냅샷 변환을 함께 바꿉니다. SuperSplat의 PLY 가져오기 회전도 처리합니다."),
    DE("Ändert die Aufwärtsachse der Vorschau und die Koordinatenumwandlung des Schnappschusses. Die PLY-Importdrehung von SuperSplat wird berücksichtigt."),
    FR("Modifie l’axe vertical de l’aperçu et la conversion du snapshot. La rotation d’import PLY de SuperSplat est prise en compte."),
    ES("Cambia el eje vertical de la vista previa y la conversión de la instantánea. También contempla la rotación de importación PLY de SuperSplat."),
    PT("Altera o eixo vertical da prévia e a conversão do instantâneo. Também considera a rotação de importação PLY do SuperSplat."),
    IT("Cambia l’asse verticale dell’anteprima e la conversione dell’istantanea. Tiene conto della rotazione d’importazione PLY di SuperSplat."),
    NL("Wijzigt de opwaartse as van de voorbeeldweergave en de conversie van de momentopname. Houdt rekening met de PLY-importrotatie van SuperSplat."),
    RU("Меняет вертикальную ось просмотра и преобразование снимка. Учитывает поворот PLY при импорте в SuperSplat."),
    TR("Önizlemenin yukarı eksenini ve anlık görüntü dönüşümünü değiştirir. SuperSplat'in PLY içe aktarma dönüşünü de hesaba katar."));
SS_MSG(snapshot_rotation,
    EN("Rotation (degrees)"),
    JA("回転（度）"),
    ZH_HANS("旋转（度）"),
    ZH_HANT("旋轉（度）"),
    ...
    RU("Поворот (градусы)"),
    TR("Döndürme (derece)"));
SS_MSG(snapshot_rotation_help,
    EN("Rotate about fixed X, then Y, then Z axes. Preview and snapshot use the same rotation, including camera leveling. Reset view keeps these settings."),
    JA("固定軸 X、Y、Z の順に回転します。プレビューとスナップショットは水平補正を含む同じ回転を使用します。視点リセットでも設定は保持されます。"),
    ZH_HANS("依次绕固定 X、Y、Z 轴旋转。预览和快照使用相同的旋转，包含相机水平校正。重置视角保留这些设置。"),
    ZH_HANT("依次繞固定 X、Y、Z 軸旋轉。預覽與快照使用相同的旋轉，包含相機水平校正。重設視角保留這些設定。"),
    KO("고정 X, Y, Z 축 순서로 회전합니다. 미리보기와 스냅샷은 수평 보정을 포함한 동일한 회전을 사용합니다. 뷰를 초기화해도 설정은 유지됩니다。"),
    DE("Um die festen Achsen X, Y, dann Z drehen. Vorschau und Schnappschuss verwenden dieselbe Drehung samt Kameraausrichtung. Ansicht zurücksetzen behält diese Werte."),
    FR("Rotation autour des axes fixes X, puis Y, puis Z. Aperçu et instantané partagent la rotation et la mise à niveau. Réinitialiser la vue conserve ces réglages."),
    ES("Gira sobre los ejes fijos X, Y y Z, en ese orden. Vista previa e instantánea comparten rotación y nivelación. Restablecer la vista conserva estos valores."),
    PT("Gira nos eixos fixos X, Y e Z, nessa ordem. Prévia e instantâneo compartilham rotação e nivelamento. Redefinir a vista mantém os valores."),
    IT("Ruota sugli assi fissi X, Y e Z, in ordine. Anteprima e istantanea condividono rotazione e livellamento. Reimpostare la vista mantiene i valori."),
    NL("Draai om vaste assen X, dan Y, dan Z. Voorbeeld en momentopname gebruiken dezelfde rotatie en nivellering. Weergave herstellen behoudt de instellingen."),
    RU("Поворот вокруг неподвижных осей X, затем Y, затем Z. Просмотр и снимок используют одинаковый поворот с выравниванием камер. Сброс вида сохраняет настройки."),
    TR("Sabit X, ardından Y ve Z eksenleri etrafında döndürür. Önizleme ve anlık görüntü, kamera hizalama dahil aynı dönüşü kullanır. Görünümü sıfırlamak ayarları korur."));
SS_MSG(snapshot_center_help,
    EN("The selected dataset camera or sparse-point center becomes the snapshot origin. It does not use the evolving splat bounds."),
    JA("選択したデータセットのカメラまたは疎な点群の中心が出力原点になります。学習中のスプラット範囲は使いません。"),
    ZH_HANS("所选的数据集相机中心或稀疏点云中心会成为快照的坐标原点，不使用训练中变化的泼溅包围盒。"),
    ZH_HANT("所選的資料集相機中心或稀疏點雲中心會成為快照的座標原點，不使用訓練中變化的潑濺包圍盒。"),
    KO("선택한 데이터셋 카메라 또는 희소 점군의 중심이 스냅샷 원점이 됩니다. 학습 중 변하는 스플랫 경계는 사용하지 않습니다."),
    DE("Das gewählte Kamera- oder Punktwolkenzentrum des Datensatzes wird zum Ursprung. Die veränderlichen Splat-Grenzen werden nicht verwendet."),
    FR("Le centre choisi des caméras ou des points épars devient l'origine. Les limites évolutives des splats ne sont pas utilisées."),
    ES("El centro elegido de cámaras o puntos dispersos será el origen. No se usan los límites cambiantes de los splats."),
    PT("O centro escolhido das câmeras ou pontos esparsos será a origem. Não usa os limites variáveis dos splats."),
    IT("Il centro scelto delle fotocamere o dei punti sparsi diventa l'origine. Non usa i limiti variabili degli splat."),
    NL("Het gekozen camera- of puntenwolkcentrum wordt de oorsprong. De veranderende splatgrenzen worden niet gebruikt."),
    RU("Выбранный центр камер или разреженных точек станет началом координат. Изменяющиеся границы сплатов не используются."),
    TR("Seçilen kamera veya seyrek nokta merkezi koordinat başlangıcı olur. Değişen splat sınırları kullanılmaz."));
SS_MSG(snapshot_export,
    ...
    ES("Generar instantánea"),
    PT("Gerar instantâneo"),
    IT("Crea istantanea"),
    NL("Momentopname maken"),
    RU("Создать снимок"),
    TR("Anlık görüntü oluştur"));
SS_MSG(snapshot_exporting,
    EN("Saving snapshot…"),
    JA("保存中…"),
    ZH_HANS("正在保存快照…"),
    ZH_HANT("正在儲存快照…"),
    KO("스냅샷 저장 중…"),
    DE("Schnappschuss wird gespeichert…"),
    FR("Enregistrement…"),
    ES("Guardando instantánea…"),
    PT("Salvando instantâneo…"),
    IT("Salvataggio istantanea…"),
    NL("Momentopname opslaan…"),
    RU("Сохранение снимка…"),
    TR("Anlık görüntü kaydediliyor…"));
SS_MSG(snapshot_export_help,
    EN("Save the current model to a new snapshot folder in the training output, using the displayed center and rotation. Keeps scene size, the live model and original checkpoints. Also works while paused or after training."),
    JA("表示中の中心と回転で、学習出力内の新しいフォルダへ現在のモデルを保存します。大きさ、学習モデル、元のチェックポイントは保持します。一時停止中や学習後も使えます。"),
    ZH_HANS("按当前中心和旋转，将此刻模型保存到训练输出目录下的新快照文件夹。保持场景尺寸，不改动训练模型和原始检查点。暂停时、训练结束后也可导出。"),
    ZH_HANT("按目前中心與旋轉，將此刻模型儲存到訓練輸出目錄下的新快照資料夾。保留場景尺寸，不更動訓練模型和原始檢查點。暫停時、訓練結束後也可匯出。"),
    KO("현재 중심과 회전으로 학습 출력의 새 폴더에 모델을 저장합니다. 크기, 학습 모델, 원본 체크포인트는 유지됩니다. 일시 중지 또는 학습 완료 후에도 가능합니다."),
    DE("Das aktuelle Modell mit angezeigtem Zentrum und Drehung in einem neuen Ausgabeordner speichern. Größe, Trainingsmodell und Checkpoints bleiben erhalten. Auch pausiert oder nach dem Training verfügbar."),
    FR("Enregistrer le modèle avec le centre et la rotation affichés dans un nouveau dossier de sortie. Taille, modèle actif et checkpoints conservés. Disponible en pause et après l'entraînement."),
    ES("Guarda el modelo con el centro y rotación mostrados en una nueva carpeta de salida. Conserva tamaño, modelo activo y checkpoints. Funciona en pausa y al terminar."),
    PT("Salva o modelo com centro e rotação exibidos em nova pasta de saída. Preserva tamanho, modelo ativo e checkpoints. Funciona em pausa e após o treino."),
    IT("Salva il modello con centro e rotazione mostrati in una nuova cartella di output. Mantiene dimensioni, modello attivo e checkpoint. Disponibile in pausa e dopo l'addestramento."),
    NL("Sla het model met het getoonde centrum en de rotatie op in een nieuwe uitvoermap. Grootte, actief model en checkpoints blijven behouden. Ook tijdens pauze en na training."),
    RU("Сохранить модель с выбранным центром и поворотом в новую папку результатов. Размер, обучаемая модель и исходные чекпойнты сохраняются. Доступно на паузе и после обучения."),
    TR("Modeli gösterilen merkez ve dönüşle çıktı dizininde yeni bir klasöre kaydeder. Boyut, etkin model ve kontrol noktaları korunur. Duraklatıldığında ve eğitim sonrasında da kullanılabilir."));
SS_MSG(snapshot_saved,
    EN("Snapshot saved: {0}"),
    JA("保存先: {0}"),
    ZH_HANS("快照已保存：{0}"),
    ZH_HANT("快照已儲存：{0}"),
    KO("스냅샷 저장됨: {0}"),
    DE("Schnappschuss gespeichert: {0}"),
    FR("Instantané enregistré : {0}"),
    ES("Instantánea guardada: {0}"),
    PT("Instantâneo salvo: {0}"),
    IT("Istantanea salvata: {0}"),
    NL("Momentopname opgeslagen: {0}"),
    RU("Снимок сохранён: {0}"),
    TR("Anlık görüntü kaydedildi: {0}"));
SS_MSG(snapshot_failed,
    EN("Snapshot failed: {0}"),
    JA("保存に失敗: {0}"),
    ZH_HANS("快照保存失败：{0}"),
    ZH_HANT("快照儲存失敗：{0}"),
    KO("스냅샷 실패: {0}"),
    DE("Schnappschuss fehlgeschlagen: {0}"),
    FR("Échec de l'instantané : {0}"),
    ES("Error de instantánea: {0}"),
    PT("Falha no instantâneo: {0}"),
    IT("Istantanea non riuscita: {0}"),
    NL("Momentopname mislukt: {0}"),
    RU("Ошибка снимка: {0}"),
    TR("Anlık görüntü başarısız: {0}"));
SS_MSG(source_folder,
    EN("Folder"), JA("フォルダ"), ZH_HANS("文件夹"), ZH_HANT("資料夾"),
    KO("폴더"), DE("Ordner"), FR("Dossier"), ES("Carpeta"), PT("Pasta"),
    IT("Cartella"), NL("Map"), RU("Папка"), TR("Klasör"));
SS_MSG(source_images,
    EN("Images"), JA("画像数"), ZH_HANS("图片数"), ZH_HANT("圖片數"),
    KO("이미지 수"), DE("Bilder"), FR("Images"), ES("Imágenes"), PT("Imagens"),
    IT("Immagini"), NL("Beelden"), RU("Изображения"), TR("Görüntüler"));
SS_MSG(source_strength,
    EN("Weight"), JA("重み"), ZH_HANS("权重"), ZH_HANT("權重"),
    KO("가중치"), DE("Gewicht"), FR("Poids"), ES("Peso"), PT("Peso"),
    IT("Peso"), NL("Gewicht"), RU("Вес"), TR("Ağırlık"));
SS_MSG(source_add_stage,
    EN("Add stage"), JA("段階を追加"), ZH_HANS("添加阶段"), ZH_HANT("新增階段"),
    KO("단계 추가"), DE("Phase hinzufügen"), FR("Ajouter une phase"),
    ES("Añadir etapa"), PT("Adicionar fase"), IT("Aggiungi fase"),
    NL("Fase toevoegen"), RU("Добавить этап"), TR("Aşama ekle"));
SS_MSG(source_need_weight,
    EN("Each stage needs a positive weight on a camera group with training images."),
    JA("各段階には学習画像を含むカメラグループの正の重みが必要です。"),
    ZH_HANS("每个阶段至少要有一个包含训练图片的相机组权重大于 0。"),
    ZH_HANT("每個階段至少要有一個包含訓練圖片的相機組權重大於 0。"),
    KO("각 단계에는 학습 이미지가 있는 카메라 그룹의 양수 가중치가 필요합니다."),
    DE("Jede Phase braucht ein positives Gewicht für eine Kameragruppe mit Trainingsbildern."),
    FR("Chaque phase exige un poids positif pour un groupe de caméras avec des images d'entraînement."),
    ES("Cada etapa requiere un peso positivo para un grupo de cámaras con imágenes de entrenamiento."),
    PT("Cada fase exige um peso positivo para um grupo de câmeras com imagens de treino."),
    IT("Ogni fase richiede un peso positivo per un gruppo di fotocamere con immagini di addestramento."),
    NL("Elke fase vereist een positief gewicht voor een cameragroep met trainingsbeelden."),
    RU("На каждом этапе нужен положительный вес группы камер с обучающими изображениями."),
    TR("Her aşamada eğitim görüntüleri içeren bir kamera grubunun ağırlığı pozitif olmalıdır."));
...
SS_MSG(opt_resolution_limit,
    EN("max side"), JA("長辺上限"), ZH_HANS("最长边上限"), ZH_HANT("最長邊上限"),
    KO("긴 변 제한"), DE("Längste Seite"), FR("Côté maximal"),
    ES("Lado máximo"), PT("Lado máximo"), IT("Lato massimo"),
    NL("Maximale zijde"), RU("Максимальная сторона"), TR("En uzun kenar"));
